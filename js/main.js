$(document).ready(function () {
  ...
  ScrollTrigger.batch(".header-ani", {
    //прост присвоїв клас анімованим елементам і в бач їх, щоб не паритись про роздільну  мацію
    ...
    onEnter: (batch) =>
      gsap.to(batch, {
        ...
        duration: 1,
        stagger: 0.2,
        // delay: 1, //якщо плануєш вставляти прелоадер
      }),

    start: "100px bottom", // запуск анімації, коли низ  екрану пройшов 100px до цілі
  });
  ...
  ScrollTrigger.batch(".change-ani", {
    // interval: 0.1, // time window (in seconds) for batching to occur.
    // batchMax: 3,   // maximum batch size (targets)
    onEnter: (batch) =>
      gsap.to(batch, {
  ...
  ScrollTrigger.batch(".applic-ani", {
    // interval: 0.1, // time window (in seconds) for batching to occur.
    // batchMax: 3,   // maximum batch size (targets)
    onEnter: (batch) =>
      gsap.to(batch, {
  ...
  gsap.from(".term-text-wrapper", {
    x: 150,
    opacity: 0,
    duration: 0.8,
    ease: "power1.inOut",
    scrollTrigger: {
      trigger: ".term-text-wrapper",
      start: "100px bottom",
      // markers: true,
    },
  });
  ...
  const updateOffset = () => header.offsetHeight;
  let options = {
    offset: updateOffset(),
    tolerance: {
      up: 10,
  ...
  if ($(".catalog").length) {
    ...
    function calculateSum() {
      let sum = 0;
      form.querySelectorAll(".tab-content input.input-rent").forEach((button) => {
        if (button.checked) {
    ...
    const totalSumElement = document.querySelector(".total-sum span");
    totalSumElement.textContent = sum + " €";

    form.addEventListener("change", () => {
      const sum = calculateSum(form);
      totalSumElement.textContent = sum + " €";
      console.log(sum);
    });
    ...
    document.querySelectorAll(".tab-button").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        let checkedInput = document.querySelector('input[name="groupindex"]:checked');
        if (checkedInput) {
          checkedInput.checked = false;
        }

        // form.querySelectorAll(".input-rent:checked").forEach((button) => {
    ...
    const rentBtns = document.querySelectorAll(".rent-me");
    ...
    rentBtns.forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        document.querySelector(".product-popup-content").innerHTML = getCardContent;

        const tabArray1 = originalElements1.split(",");
        const tabArray2 = originalElements2.split(",");
        const tabArray3 = originalElements3.split(",");
        const tabArray4 = originalElements4.split(",");
        tabArray1.pop();
        tabArray2.pop();
        tabArray3.pop();
        tabArray4.pop();
        const tab1 = document.querySelector("#tab1 .tab-inner");
        const tab2 = document.querySelector("#tab2 .tab-inner");
        const tab3 = document.querySelector("#tab3 .tab-inner");
        const tab4 = document.querySelector("#tab4 .tab-inner");
        tab1.innerHTML = "";
        tab2.innerHTML = "";
        tab3.innerHTML = "";
        tab4.innerHTML = "";

        const tabLabels = ["З обмеженням пробігу та з франшизою", "З обмеженням пробігу та без франшизою", "Без обмеження пробігу та з франшизою", "Без обмеження пробігу та без франшизи"];

        tabArray1.forEach((score, index) => {
          tab1.innerHTML +=
            ' <input type="radio" class="input-rent" name="groupindex" value="' +
            score +
            '" id="group1-option' +
            index +
            '" /><label for="group1-option' +
            index +
            '">' +
            tabLabels[index] +
            "</label>";
        });
        tabArray2.forEach((score, index) => {
          tab2.innerHTML +=
            ' <input type="radio" class="input-rent" name="groupindex" value="' +
            score +
            '" id="group2-option' +
            index +
            '2" /><label for="group2-option' +
            index +
            '2">' +
            tabLabels[index] +
            "</label>";
        });
        tabArray3.forEach((score, index) => {
          tab3.innerHTML +=
            ' <input type="radio" class="input-rent" name="groupindex" value="' +
            score +
            '" id="group3-option' +
            index +
            '3" /><label for="group3-option' +
            index +
            '3">' +
            tabLabels[index] +
            "</label>";
        });
        tabArray4.forEach((score, index) => {
          tab4.innerHTML +=
            ' <input type="radio" class="input-rent" name="groupindex" value="' +
            score +
            '" id="group4-option' +
            index +
            '4" /><label for="group4-option' +
            index +
            '4">' +
            tabLabels[index] +
            "</label>";
  ...
  if ($("input[name='your-tel']").length) {
    $("input[name='your-tel']").mask("+38 999 999 99 99", {
      completed: function () {
  ...
  $(".property").click(function () {
    let getProperty = $(this).closest(".property");
    let propertyTitle = getProperty.attr("data-title");
    let propertyPrice = getProperty.attr("data-price");
    let propertyLink = getProperty.attr("data-link");

    $("#propertyTitle").val(propertyTitle);
    $("#propertyPrice").val(propertyPrice);
    $("#propertyLink").val(propertyLink);

    // console.log($("#propertyTitle").val());
    // console.log($("#propertyPrice").val());
    // console.log($("#propertyLink").val());
  });

  $(".comm-btn").click(function () {
    let getComm = $(".comm-card.active");
    let commTitle = getComm.attr("data-title");
    let commNum = getComm.attr("data-num");

    $("#commTitle").val(commTitle);
    $("#commNum").val(commNum);

    // console.log($("#commTitle").val());
    // console.log($("#commNum").val());
  });
  ...
  if ($("#single-content-buy").length) {
    ...
    function Ascroll() {
      let Ra = a.getBoundingClientRect(),
        R1bottom = document.querySelector(".single-content-holder").getBoundingClientRect().bottom - 30;
